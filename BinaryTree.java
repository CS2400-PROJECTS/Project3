public class BinaryTree<T> implements BinaryTreeInterface<T>
{
   ...
   {
	   // call pospostorderTraverse(BinaryNode<T> node) and pass the root node
   ...
   {
	   // this will be the recursive method using a given node
	   if(node!=null) //check if node is empty
	   {
		  // for post oder we go do left, right, and root
		  // so we start and get the left 
		   postorderTraverse(node.getLeftChild()); 
		  // then go check and get the right
		   postorderTraverse(node.getRightChild());
		  // Print the sequence that we get from the postOrder
   ...
   public void postorderTraverse_callBinaryNodeMethod()
   {
	   ...
	   // Call postorderTraverse_binaryNodeMethod() which will do the 
	   // postorderTraverse and print out the sequence
  ...
  return 0;
   ...
   return Math.max(root.right,root.left) +1;
   ...
   private int getNumberOfNodes(BinaryNode<T> node)
   {
	  ...
	  nodeNumber +=(getNumberOfNodes(node.getLeftChild())+getNumberOfNodes(node.getRightChild()));
	  return nodeNumber;
   } // end getNumberOfNodes
